...
$dotnetFolderPath = Join-Path $repoRootPath ".dotnet.azure"
$expectedWarningsFilePath = Join-Path -Path $dotnetFolderPath "sdk\openai\Azure.AI.OpenAI\src\ExpectedWarnings.txt"
$libraryCsprojFilePath = Join-Path $dotnetFolderPath "sdk\openai\Azure.AI.OpenAI\src\Azure.AI.OpenAI.csproj"
$packageName = "Azure.AI.OpenAI"

Write-Host "Creating test app..."

$tempFolderPath = Join-Path $dotnetFolderPath "\TempAotCompatibility"
...
try {
    ...
    $publishOutput = dotnet publish AotCompatibility.csproj -c Unsigned -nodeReuse:false /p:UseSharedCompilation=false /p:ExposeExperimentalFeatures=true
